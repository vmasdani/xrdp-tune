#!/bin/bash
# Trade looks for latency: flat dark theme, 1-bit fonts, plain cursor, quiet panel,
...
kw --file kdeglobals --group KDE --key widgetStyle Fusion
...
# Solid colour wallpaper on every existing containment.
...
if [ -f "$APPLETS" ]; then
  ...
  grep -q 'org.kde.color' "$APPLETS" || true
fi

echo "== Fonts: no antialias, full hinting, no subpixel"
kw --file kdeglobals --group General --key XftAntialias false
...
echo "fonts antialias on, delete ~/.config/fontconfig/fonts.conf, Do Not Disturb off."
